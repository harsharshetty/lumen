...
function SignInArtwork() { return <Box data-testid="signin-artwork-panel" sx={{ position: 'relative', width: '100%', height: { xs: 'auto', md: '100%' }, aspectRatio: { xs: '5 / 6', md: 'auto' }, minWidth: 0, overflow: 'hidden', bgcolor: '#102451' }}><Box component="img" src={signinArtwork} alt="Lumen — bright learning for brighter tomorrows" sx={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block', objectFit: 'cover' }} /></Box>; }
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    const [learnerResult, curriculumResult] = await Promise.allSettled([request<LearnerSummary[]>('/api/learners'), request<CurriculumResponse[]>('/api/curricula')]);
    if (isUnauthenticated(learnerResult) || isUnauthenticated(curriculumResult)) { setLearnerState('unauthenticated'); setCurriculumState('error'); setProfileState('error'); return; }
