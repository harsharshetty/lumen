...
function LumenMark() {
  return <Box component="span" aria-hidden="true" sx={{ display: 'inline-grid', placeItems: 'center', width: 42, height: 42, color: '#1b78ff', fontSize: 38, lineHeight: 1 }}>✦</Box>;
}

function GoogleMark() {
  return <Box component="span" aria-hidden="true" sx={{ mr: 1.5, fontSize: 25, lineHeight: 1, fontWeight: 900, fontFamily: 'Arial, sans-serif', background: 'conic-gradient(from -35deg, #4285f4 0 25%, #34a853 25% 45%, #fbbc05 45% 68%, #ea4335 68% 83%, #4285f4 83% 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>G</Box>;
}
...
function Capability({ icon, title, copy, tint }: { icon: string; title: string; copy: string; tint: string }) {
  return (
    <Stack spacing={1.15} alignItems="center" textAlign="center" sx={{ minWidth: 0 }}>
      <Box aria-hidden="true" sx={{ width: { xs: 54, sm: 66 }, height: { xs: 54, sm: 66 }, borderRadius: '50%', display: 'grid', placeItems: 'center', bgcolor: tint, fontSize: { xs: 24, sm: 30 }, boxShadow: 'inset 0 0 0 1px rgba(16,43,96,0.02)' }}>{icon}</Box>
      <Typography sx={{ fontWeight: 800, color: '#0b1f5e', fontSize: { xs: '0.9rem', sm: '1.03rem' }, lineHeight: 1.25 }}>{title}</Typography>
      <Typography sx={{ color: '#62719a', fontSize: { xs: '0.78rem', sm: '0.88rem' }, lineHeight: 1.45, maxWidth: 170 }}>{copy}</Typography>
    </Stack>
  );
}
...
function SignInLanding() {
  return (
    <Box component="main" sx={{ minHeight: '100vh', p: { xs: 1.5, sm: 3, md: 4 }, display: 'grid', placeItems: 'center', overflowX: 'hidden', background: 'radial-gradient(circle at 15% 12%, rgba(255,255,255,.95), transparent 27%), radial-gradient(circle at 92% 86%, rgba(203,219,249,.85), transparent 25%), #eef4ff' }}>
      <Box sx={{ width: 'min(1330px, 94vw)', minHeight: { xs: 760, md: 'min(795px, calc(100vh - 54px))' }, display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, borderRadius: { xs: 4, md: '56px' }, overflow: 'hidden', bgcolor: '#fff', boxShadow: '0 34px 90px rgba(44, 72, 125, 0.18)', border: '1px solid rgba(116, 143, 201, 0.10)' }}>
        <SignInArtwork />
        <Stack sx={{ minWidth: 0, px: { xs: 4, sm: 6, md: 7.5 }, py: { xs: 5, md: 6.5 }, bgcolor: '#fff' }}>
          <Stack direction="row" spacing={1.1} alignItems="center"><LumenMark /><Typography sx={{ fontSize: { xs: '1.8rem', md: '2.1rem' }, fontWeight: 800, color: '#0b1f5e', letterSpacing: '-0.03em' }}>Lumen</Typography></Stack>
          <Box sx={{ mt: { xs: 7, md: 10 } }}>
            <Typography component="h1" sx={{ fontSize: { xs: '2.45rem', md: '3.25rem' }, fontWeight: 800, letterSpacing: '-0.035em', lineHeight: 1.05, color: '#0b1f5e' }}>Welcome back</Typography>
            <Typography sx={{ mt: 1.05, color: '#62719a', fontSize: { xs: '1.05rem', md: '1.24rem' } }}>Sign in to your Lumen account.</Typography>
          </Box>
          <Button variant="outlined" href="/oauth2/authorization/google" sx={{ mt: 4.5, minHeight: 72, borderRadius: 2.5, borderColor: '#d7e0f0', color: '#0b1f5e', bgcolor: '#fff', fontSize: { xs: '1rem', md: '1.1rem' }, fontWeight: 800, boxShadow: '0 7px 18px rgba(46, 79, 144, 0.06)', '&:hover': { borderColor: '#b9c9e6', bgcolor: '#fff' } }}><GoogleMark />Continue with Google</Button>
          ...
          <Stack direction="row" spacing={2.3} sx={{ mt: 'auto', pt: { xs: 6, md: 7.5 }, alignItems: 'center' }}>
            <Typography component="a" href="#" sx={{ color: '#1677ff', textDecoration: 'none', fontSize: '0.86rem' }}>Privacy</Typography><Typography sx={{ color: '#b3bfd4' }}>|</Typography><Typography component="a" href="#" sx={{ color: '#1677ff', textDecoration: 'none', fontSize: '0.86rem' }}>Terms</Typography><Typography sx={{ color: '#b3bfd4' }}>|</Typography><Typography component="a" href="#" sx={{ color: '#1677ff', textDecoration: 'none', fontSize: '0.86rem' }}>Help</Typography>
          </Stack>
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [loadedLearners, loadedCurricula] = await Promise.all([request<LearnerSummary[]>('/api/learners'), request<CurriculumResponse[]>('/api/curricula')]);
      const selections = await Promise.all(loadedLearners.map(async (learner) => [learner.id, (await request<CurriculumResponse[]>(`/api/learners/${learner.id}/curricula`)).map((item) => item.id)] as const));
      ...
      if (status === 401) { setLearnerState('unauthenticated'); setCurriculumState('error'); setProfileState('error'); return; }
      const nextState: FlowState = status === 403 ? 'forbidden' : 'error';
      setLearnerState(nextState); setCurriculumState(nextState); setProfileState(nextState);
    }
  }, []);
  ...
  const createLearner = async (displayName: string) => {
    const learner = await request<LearnerSummary>('/api/learners', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ displayName }) });
