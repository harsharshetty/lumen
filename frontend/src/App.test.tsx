...
describe('App', () => {
  ...
  it('handles an empty first-time account', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(jsonResponse([])).mockResolvedValueOnce(jsonResponse([])); render(<App />);
  ...
  it('shows the frozen branded sign-in entry when any bootstrap API reports unauthenticated', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(jsonResponse([])).mockResolvedValueOnce(emptyResponse(401)); render(<App />);
  ...
  it('keeps learners available when curriculum catalog loading fails', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(jsonResponse([{ id: 'learner-1', displayName: 'Ava' }])).mockResolvedValueOnce(emptyResponse(500)).mockResolvedValueOnce(jsonResponse([])); render(<App />);
  ...
  it('keeps curriculum state ready when learner loading fails', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(emptyResponse(403)).mockResolvedValueOnce(jsonResponse([])); render(<App />);
  ...
  it.each([[403, 'ready:ready:forbidden'], [500, 'ready:ready:error']])('isolates learner selection failure %s to profile state', async (status, expected) => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(jsonResponse([{ id: 'learner-1', displayName: 'Ava' }])).mockResolvedValueOnce(jsonResponse([])).mockResolvedValueOnce(emptyResponse(status)); render(<App />);
  ...
  it('returns to sign-in when learner selection loading reports an expired session', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(jsonResponse([{ id: 'learner-1', displayName: 'Ava' }])).mockResolvedValueOnce(jsonResponse([])).mockResolvedValueOnce(emptyResponse(401)); render(<App />);
