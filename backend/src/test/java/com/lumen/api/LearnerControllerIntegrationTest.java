...
class LearnerControllerIntegrationTest {
    ...
    void authenticatedUserCreatesOwnLearnerThenListsAndReadsIt() throws Exception {
        ...
        String response = mockMvc.perform(post("/api/learners").with(authenticatedAs("parent-subject")).with(csrf())
                        .contentType(MediaType.APPLICATION_JSON).content("{\"displayName\":\"Ava\"}"))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.displayName").value("Ava"))
                .andReturn().getResponse().getContentAsString();
        Learner learner = learnerRepository.findAll().getFirst();
        assertThat(response).contains(learner.getId().toString());
        assertThat(accessRepository.findAll()).singleElement().satisfies(access -> {
        ...
        mockMvc.perform(get("/api/learners").with(authenticatedAs("parent-subject"))).andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1)).andExpect(jsonPath("$[0].id").value(learner.getId().toString()))
        ...
        mockMvc.perform(get("/api/learners/{id}", learner.getId()).with(authenticatedAs("parent-subject")))
                .andExpect(status().isOk()).andExpect(jsonPath("$.id").value(learner.getId().toString()))
                .andExpect(jsonPath("$.displayName").value("Ava"));
    }

    @Test
    void invalidLearnerWriteReturnsBadRequestWithoutPersistingLearnerOrAccess() throws Exception {
        provisionUser("parent-subject", "Parent");
        long learnerCount = learnerRepository.count();
        long accessCount = accessRepository.count();
        mockMvc.perform(post("/api/learners").with(authenticatedAs("parent-subject")).with(csrf())
                        .contentType(MediaType.APPLICATION_JSON).content("{\"displayName\":\"   \"}"))
                .andExpect(status().isBadRequest());
        assertThat(learnerRepository.count()).isEqualTo(learnerCount);
        assertThat(accessRepository.count()).isEqualTo(accessCount);
    }
    ...
    void unauthenticatedAndUnrelatedUsersCannotAccessLearnerData() throws Exception {
        ...
        mockMvc.perform(get("/api/learners").with(authenticatedAs("other-subject"))).andExpect(status().isOk())
    ...
    void learnerCurriculumSelectionHonorsAccessLevelsAndActiveCatalog() throws Exception {
        ...
        active.setActive(true); active = curriculumRepository.save(active);
        ...
        mockMvc.perform(post("/api/learners/{learnerId}/curricula/{curriculumId}", learner.getId(), active.getId())
                        .with(authenticatedAs("contributor-subject")).with(csrf())).andExpect(status().isOk());
        mockMvc.perform(post("/api/learners/{learnerId}/curricula/{curriculumId}", learner.getId(), active.getId())
                        .with(authenticatedAs("owner-subject")).with(csrf())).andExpect(status().isOk());
        assertThat(learnerCurriculumRepository.findByLearnerId(learner.getId())).hasSize(1);
        mockMvc.perform(get("/api/learners/{learnerId}/curricula", learner.getId()).with(authenticatedAs("viewer-subject")))
                .andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(1));
        mockMvc.perform(post("/api/learners/{learnerId}/curricula/{curriculumId}", learner.getId(), inactive.getId())
                        .with(authenticatedAs("owner-subject")).with(csrf())).andExpect(status().isBadRequest());
        mockMvc.perform(post("/api/learners/{learnerId}/curricula/{curriculumId}", learner.getId(), active.getId())
                        .with(authenticatedAs("viewer-subject")).with(csrf())).andExpect(status().isForbidden());
        assertThat(learnerCurriculumRepository.findByLearnerId(learner.getId())).hasSize(1);
        mockMvc.perform(get("/api/learners/{learnerId}/curricula", learner.getId()).with(authenticatedAs("other-subject")))
                .andExpect(status().isForbidden());
        mockMvc.perform(delete("/api/learners/{learnerId}/curricula/{curriculumId}", learner.getId(), active.getId())
                        .with(authenticatedAs("contributor-subject")).with(csrf())).andExpect(status().isNoContent());
        assertThat(learnerCurriculumRepository.findByLearnerId(learner.getId())).isEmpty();
        mockMvc.perform(delete("/api/learners/{learnerId}/curricula/{curriculumId}", learner.getId(), active.getId())
                        .with(authenticatedAs("owner-subject")).with(csrf())).andExpect(status().isNotFound());
    }
    ...
    private RequestPostProcessor authenticatedAs(String subject) {
        OidcUser principal = mock(OidcUser.class); when(principal.getSubject()).thenReturn(subject);
