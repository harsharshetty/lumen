...
The status below reflects `main` after Playwright foundation PR #59. Work that exists only in an open PR is not counted as protected on `main`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| #19 / ADR-002 | One `(provider, providerSubject)` identifies one Lumen User; email/name are not identity keys | Identity | Release-blocking | domain + persistence + server integration + E2E | `LoginIdentityDomainTest`, `IdentityAccessDomainTest`, repository tests cover model/persistence portions | Partial | Runtime authentication/resolution in #19; server proof in #19/#32; browser journey #27 |
| #19 / ADR-002 | Different OAuth providers remain different Lumen Users even when email/name matches | Identity isolation | Release-blocking | server integration + E2E | Domain representation exists; no runtime cross-provider behavioral proof on `main` yet | Partial | #19 runtime boundary, then #27 |
| #19 | Unauthenticated caller cannot use protected application APIs | Authentication | Release-blocking | server integration + E2E | Premature learner routes remain absent; this is not yet proof of the final auth boundary | Not yet implemented | #19 runtime boundary; #27 E2E |
| #19 | First login provisions exactly one USER + LoginIdentity; repeat login resolves the same User | Identity lifecycle | Release-blocking | server integration + persistence + E2E | Model/repositories exist only | Partial | #19 runtime behavior; #33 persistence; #27 E2E |
| #19 | USER vs ADMIN authorization is enforced server-side | Platform authorization | Release-blocking | server integration + E2E where user-visible | Role model exists; enforcement not implemented | Not yet implemented | #19/#21, #32, #28 |
| #20 / ADR-002 | User can access only learners linked through `UserLearnerAccess`; no global learner enumeration | Learner privacy | Release-blocking | server integration + E2E | `UserLearnerAccess` domain/schema exists; unsafe V0 learner REST exposure was removed and route-absence regression is covered by `LearnerApiExposureIntegrationTest` | Partial | #19 + #20 enforcement; exhaustive #32; E2E #29 |
...
| #21 | USER may read active canonical catalog but cannot mutate canonical curricula; ADMIN may manage it | Curriculum administration | Release-blocking | server integration + E2E | Canonical curriculum domain/services pre-exist, but approved V1 ADMIN API boundary is not implemented | Not yet implemented | #19 + #21, #32, #28 |
| #21 | Deactivated curriculum cannot be newly selected, while existing learner associations are not silently deleted | Curriculum lifecycle | High | service/server integration + PostgreSQL + E2E | Approved behavior specified, final admin/onboarding flow not implemented | Not yet implemented | #21/#23, #33, #28/#30 |
| #22 | Only ADMIN receives/uses curriculum-management UI; backend remains authoritative | Admin UI | Release-blocking | component + E2E + server integration | UI not implemented | Not yet implemented | #21 then #22; #28 |
| #23 | Authenticated learner creation makes creator OWNER; acting user comes only from authenticated context | Learner onboarding API | Release-blocking | server integration + E2E | Secured learner API intentionally absent | Not yet implemented | #19 + #20 + #21; #32/#29/#30 |
...
| #25 | No-learner authenticated user enters onboarding; returning user sees only authorized learners and persisted selections | Learner onboarding UI | High | component + E2E | UI not implemented | Not yet implemented | #19/#23 then #25; #29/#30 |
| #26 | Browser test infrastructure exercises real React + Spring Boot + datastore runtime and fails CI on E2E failure | E2E infrastructure | High | E2E/CI | `e2e/tests/smoke.spec.ts`; Playwright CI job with PostgreSQL and retained diagnostics | Protected | Foundation available for #27-#30/#34 |
| #27 | Auth and identity-isolation critical journey: unauth blocked, first/repeat login, cross-provider separation, no client impersonation | Authentication E2E | Release-blocking | E2E | Playwright infrastructure exists; feature scenarios not yet implemented | Not yet implemented | #19 complete; #26 is complete |
| #28 | ADMIN curriculum lifecycle works end-to-end; USER cannot gain mutation rights via UI or direct API | Curriculum E2E | Release-blocking | E2E + server integration | Infrastructure only | Not yet implemented | #21 + #22; #26 complete |
...
CI currently exposes backend tests/coverage, frontend tests/coverage, container build, and Playwright E2E as independent jobs/signals. Future #35 work will make behavioral test counts and remaining Partial/Unprotected release-blocking rows more visible in CI reporting. Until then, this document is the auditable behavioral-protection ledger.
